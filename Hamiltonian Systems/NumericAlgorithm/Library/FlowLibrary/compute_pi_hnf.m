function [pis] = compute_pi_hnf(n_mat,k_mat, H,N,resModes)
%%
...
pis_pos = pink(n_mat(:,n_pos_idx),k_mat(:,k_pos_idx), H,N,resModes);
...
function [pis] = pink(n_mat,K,H,N,resModes)
...
for f = 1:z_k
    ...
    for i = 1:size(sz_un,2)
        ...
        resModes = [];
        if ~isempty(resModes)
            % Only works for 2D LSM so far
            % move coord directions [1,2] of multi index to resModes
            LSM1 = find(n_idx_row == 1);
            LSM2 = find(n_idx_row == 2);
            n_idx_row(LSM1) = resModes(1)* ones(1,numel(LSM1));
            n_idx_row(LSM2) = resModes(2)* ones(1,numel(LSM2));
        end
        %%
