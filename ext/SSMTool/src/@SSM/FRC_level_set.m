...
for j = 1:nPar
    ...
    if obj.Options.contribNonAuto
        ...
        [W1, R1] = obj.compute_perturbed_whisker(order-1,W0,R0);
        %     compute grid for rho and psi
        f =  nonzeros(R1(1).R(1).coeffs); % leading-order modal forcing coefficient
    else
        if j == 1
            obj.System.Omega = imag(lambda);
            [~, R1] = obj.compute_perturbed_whisker(0,[],[]);
            obj.System.Omega = Omega;
            f =  nonzeros(R1(1).R(1).coeffs); % leading-order modal forcing coefficient
        end
        W1 = [];
    ...
    [rho, psi, RHO, PSI] = compute_polar_grid(omegaRange,epsilon,gamma,lambda,f,rhoScale, nRho, nPsi);
